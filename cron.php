	...
	if ( isset($sturents_options['feeds']) && is_array($sturents_options['feeds']) && !empty($sturents_options['feeds']) )
    {
    	...
    	foreach ( $sturents_options['feeds'] as $i => $feed )
    	{
    		...
    		if ( $ok_to_run_import )
    		{
				...
				$params = array( 'landlord' => $feed['landlord_id'], 'public' => $feed['public_key'] );
					    	...
					    	if ( isset($branch['properties']) && is_array($branch['properties']) && !empty($branch['properties']) )
							{
								foreach ($branch['properties'] as $property) 
								{
									...
									if ( $inserted_updated )
									{
										...
										// Clean price
										$price = round(preg_replace("/[^0-9.]/", '', $property['contract']['price']['amount']));

										update_post_meta( $post_id, '_rent', $price );

										$rent_frequency = 'pcm';
										$price_actual = $price;
										if ( $property['contract']['price']['amount_per'] == 'person' )
										{
											switch ($property['contract']['price']['time_period'])
											{
												case "week":
												{ 
													$rent_frequency = 'pppw';
										...
										update_post_meta( $post_id, '_deposit', $property['contract']['deposit']['amount'] );

										$available_date = '';
										if ( isset($property['available_from']) && $property['available_from'] != '' )
										{
											$explode_available_from = explode("/", $property['available_from']);
											if ( count($explode_available_from) == 3 )
											{
												$available_date = $explode_available_from[2] . '-' . $explode_available_from[1] . '-' . $explode_available_from[0];
											}
										}
								        ...
								        // Media - Images
										//$this->add_log( 'Importing photos', (string)$property->propertyID );
										...
										// Media - Floorplans
										//$this->add_log( 'Importing floorplans', (string)$property->propertyID );
										...
										//$this->add_log( 'Successfully imported ' . count($media_ids) . ' floorplans', (string)$property->propertyID );

										// Media - EPCs
										//$this->add_log( 'Importing EPCs', (string)$property->propertyID );
										...
										//$this->add_log( 'Successfully imported ' . count($media_ids) . ' EPCs', (string)$property->propertyID );
